...
struct Transaction: Displayable{
    ...
    mutating func markAsPending(pendingStatus: Bool){
...
//Test
var t1 = Transaction(id: "1", date: Date.now, amount: 2_500.00, description: "Direct Deposit", isDebit: false, isPending: false)
t1.printDetails()
...
let transactions: [Displayable] = [
    Transaction(id: "123", date: Date.now, amount: 5_500.00, description: "Venmo Transfer", isDebit: false, isPending: false),
    Transaction(id: "456", date: Date.now, amount: 1_500.00, description: "Dunkin Donuts", isDebit: true, isPending: false)
]
...
let accDashboard1 = AccountDashboard(dataSource: MockAccountDataSource())
print("MockAccount:")
accDashboard1.showSummary(for: "123")
let accDashboard2 = AccountDashboard(dataSource: LiveAccountDataSource())
print("LiveAccount:")
accDashboard2.showSummary(for: "456")
...
//One should NEVER force unwrap user input like the example above. 
//This pattern of unwrapping will cause a runtime error if the value is nil.
//Instead use a different optional unwrapping pattern such as if let.
...
do {
    let result = try executeTransfer(amount: 0.00, fromBalance: 2_000.00, toAccountId: "A123", 
                    dailyUsed: 1_000.00, dailyLimit: 8_000.00)
    print(result)
...
do {
    let result = try executeTransfer(amount: 100, fromBalance: 2_000.00, toAccountId: "", 
                    dailyUsed: 1_000.00, dailyLimit: 8_000.00)
    print(result)
} catch let e as TransferError {
    print(e.localizedDescription)
...
do {
    let result = try executeTransfer(amount: 4_000.00, fromBalance: 2_000.00, toAccountId: "A123", 
                    dailyUsed: 1_000.00, dailyLimit: 8_000.00)
    print(result)
} catch let e as TransferError {
    print(e.localizedDescription)
...
do {
    let result = try executeTransfer(amount: 1_500.00, fromBalance: 2_000.00, toAccountId: "A123", 
                    dailyUsed: 1_000.00, dailyLimit: 2_000.00)
    print(result)
} catch let e as TransferError {
    print(e.localizedDescription)
} 
...
do {
    let result = try executeTransfer(amount: 100.00, fromBalance: 2_000.00, toAccountId: "ERR_NET", 
                    dailyUsed: 1_000.00, dailyLimit: 2_000.00)
    print(result)
} catch let e as TransferError {
    print(e.localizedDescription)
...
do {
    let result = try executeTransfer(amount: 100.00, fromBalance: 2_000.00, toAccountId: "A123", 
                    dailyUsed: 1_000.00, dailyLimit: 2_000.00)
    print(result)
} catch let e as TransferError {
    print(e.localizedDescription)
...
let result1 = try? executeTransfer(amount: -200.00, fromBalance: 2_000.00, toAccountId: "A123", 
                    dailyUsed: 1_000.00, dailyLimit: 2_000.00)
print(result1 ?? "Transfer failed")

let result2 = try? executeTransfer(amount: 200.00, fromBalance: 2_000.00, toAccountId: "A123", 
                    dailyUsed: 1_000.00, dailyLimit: 2_000.00)
...
let ints = [3, 1, 7, 2]
let doubles = [1.03, 3.45, 2.66]
let strings = ["cleo", "honey", "mina"]

printFirst(arr: ints)
printFirst(arr: doubles)
printFirst(arr: strings)
...
transactionAmounts.pop()
print(transactionAmounts.top ?? "Empty")
print(transactionAmounts.count)
...
if let largestNum = findLargest(arr: ints), let largestDouble = findLargest(arr: doubles),
     let largestString = findLargest(arr: strings) {
    print(largestNum)
    print(largestDouble)
    print(largestString)
}