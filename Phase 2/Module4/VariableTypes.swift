...
let formattedAverage = String(format: "%.2f", averageTransaction)
let summary = "\(transactionCount) transactions averaging $\(formattedAverage) each"
print(summary)
...
if let input = parsedAmount {
    print("Parsed amount: \(input)")
} else {
    print("Invalid input")
}
...
func processWithdrawal(amount: Double, availableBalance: Double) -> String {
    ...
    guard amount < availableBalance else {
        let formattedBalance = String(format: "%.2f", availableBalance)
        return "Insufficient funds. Available \(formattedBalance)"
    }
...
//
// ============================================================
// 1. Swift requires explicit type conversion because it is a compiled language meaning if types aren't explicitly stated it will cause a compile time error.
// 2. let is used to initiate constant variables and var is used to initiate mutable variables
// 3. You can use a guard to avoid the pyramid of doom (nested if else statements)
// 4. Swift's switch statement does not fall through and it must be exhaustive 