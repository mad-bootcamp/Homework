...
fun describe(result: TransferResult): String {
    when (result) {
        is TransferResult.Success -> return "Success! Id: ${result.confirmationId}"
        is TransferResult.Failure -> return "Failure: ${result.reason}"
    }
}

println(describe(TransferResult.Success("200")))
println(describe(TransferResult.Failure("Unauthorized")))
...
suspend fun fetchAccounts(): List<Account> {
    delay(500)
    val accounts = listOf(
        Account("1", "Erica", 3000.0),
        Account("2", "Finn", -200.0),
        Account("3", "Cleo", 20000.0),
        Account("4", "Honey", -60.0)
    )
    return accounts
}
...
fun main() = runBlocking {
    ...
    val sumOfBalances = accounts.sumOf { it.balance }
    println(overdraftAccounts)
    println(sumOfBalances.asCurrency())
}
